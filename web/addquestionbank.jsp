...
<body>
    ...
    <div class="container">
        ...
        <button class="btn btn-light">
            <a href="view-all-question.jsp" style="text-decoration: none; color: Black">Trở về</a>
        </button>
        <% } else { %>
        <!-- Nếu user.role != 1 -->
        <button class="btn btn-light">
            <a href="viewuserquestion.jsp" style="text-decoration: none; color: Black">Trở về</a>
        </button>
        <% } %>
        <form action="AddQuestionBank" method="POST" enctype="multipart/form-data">
            <fieldset>

                <label for="subjectID" style="font-weight: bold">Chọn môn học:</label>
                <select name="subjectID" id="subjectID" required>
                    <option value="">Chọn môn học</option>
                    <% 
                        // Lặp qua danh sách các môn học và tạo các option
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content" style="width: 500px; margin: auto">
                    <form action="DocFileQuestionAdd" method="POST" enctype="multipart/form-data">
                        ...
                        <label for="subjectID" style="font-weight: bold">Chọn môn học:</label>
                        <select name="subjectID" id="subjectID_File" required onchange="updateSubjectID()">
                            <option value="">Chọn môn học</option>
                            <% 
                                // Lặp qua danh sách các môn học và tạo các option
                                for (Subjects subject : subjects) { 
                        ...
                        <!-- Đảm bảo subjectID được truyền vào input hidden sau khi người dùng chọn -->
                        <input type="hidden" name="subjectID" id="subjectIDHidden" value="">
    ...
    <script>
        ...
        // Lắng nghe sự kiện thay đổi khi người dùng chọn môn học
        document.getElementById('subjectID').addEventListener('change', function () {
            var subjectID = this.value;  // Lấy giá trị subjectID đã chọn
            document.getElementById('subjectIDHidden').value = subjectID;  // Gán giá trị cho input hidden
        });

        // Cập nhật giá trị của subjectID vào input hidden
        function updateSubjectID() {
            var subjectID = document.getElementById("subjectID_File").value;
            document.getElementById("subjectIDHidden").value = subjectID;
        }
